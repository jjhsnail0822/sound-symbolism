...
def generate_datasets(language, task, experiment_name):
    ...
    if LANGUAGE == "cross_language":
        ...
        for lang in LANGUAGES:
            ...
            with open(f'data/dialogues/{lang}2en.json', 'r', encoding='utf-8') as f:
                lang_dialogues = json.load(f)
            dialogues.extend(lang_dialogues)
    else:
        with open(f'data/dialogues/{LANGUAGE}2en.json', 'r', encoding='utf-8') as f:
            dialogues = json.load(f)

    with open(f'data/processed/nat/clustering/crosslingual_clustered_{LANGUAGE}.json', 'r', encoding='utf-8') as f:
    ...
    with open(f'data/prompts/understanding/non_en_pair_matching/ipa/{EXPERIMENT_NAME}-{LANGUAGE}.json', 'w', encoding='utf-8') as f:
