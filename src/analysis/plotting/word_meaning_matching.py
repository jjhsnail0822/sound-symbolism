...
EXP_NAME = 'word_meaning_matching_original'

if EXP_NAME == "word_meaning_matching_original":
    TITLE = 'Word-Meaning Pair Matching using Original Words'
    exp_dir = f'results/experiments/word_meaning_matching/original'
elif EXP_NAME == "word_meaning_matching_romanized":
    TITLE = 'Word-Meaning Pair Matching using Romanized Words'
    exp_dir = f'results/experiments/word_meaning_matching/romanized'
elif EXP_NAME == "word_meaning_matching_ipa":
    TITLE = 'Word-Meaning Pair Matching using IPA Words'
    exp_dir = f'results/experiments/word_meaning_matching/ipa'
elif EXP_NAME == "word_meaning_matching_audio":
    TITLE = 'Word-Meaning Pair Matching using Spoken Words'
    exp_dir = f'results/experiments/word_meaning_matching/audio'
else:
...
for fp in files:
    ...
    for rec in arr:
        ...
        # Category name: word_group + task type
        word_group = dp.split('-')[-1].split('.')[0].upper()
        t = '→'.join(['Word','Meaning']) if 'word_to_meaning' in dp else '→'.join(['Meaning','Word'])
        cat = f'{word_group} {t}'
        if m not in data:
            data[m] = {}
...
out_png = f'results/plots/experiments/word_meaning_matching/{EXP_NAME}.png'
