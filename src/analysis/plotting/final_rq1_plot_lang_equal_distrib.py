...
def plot_final_rq1_bar_comparison(human_eval, constructed, natural, constructed_model_scores, natural_model_scores, metric_label='macro_f1_score', input_type=''):
    ...
    two_stage_font_size = 24
    one_stage_font_size = 30
    # print(f"[DEBUG] group_data: {group_data}")
    group_names = list(group_data.keys())
    n_groups = len(group_names)
    fig, axes = plt.subplots(1, n_groups, figsize=(25, 15), sharey=False)
    ...
    if handles:
        ...
        fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 0.08), ncol=len(handles), fontsize=one_stage_font_size-6)
...
def plot_input_type(
    ...
    # --- Save Plot ---
    save_path_png = f"{save_path_prefix}/input_type_scatter_lang_avg.png"
    plt.savefig(save_path_png, bbox_inches='tight')
    print(f"Saved scatter plot to {save_path_png}")
    save_path_pdf = f"{save_path_prefix}/input_type_scatter_lang_avg.pdf"
...
def aggregate_per_model_by_input_type(json_path, metric='macro_f1_score'):
    ...
    all_langs = natural_languages + constructed_languages

    for lang in all_langs:
        if lang not in data:
            continue
        
        word_type_to_process = 'natural' if lang in natural_languages else 'constructed'
        
        for model_name, model_data in data[lang].items():
            if model_name not in model_scores:
                model_scores[model_name] = {}

            if word_type_to_process not in model_data:
                continue

            for input_type, input_type_data in model_data[word_type_to_process].items():
                ...
                if word_type_to_process not in model_scores[model_name][input_type]:
                    model_scores[model_name][input_type][word_type_to_process] = {}
                ...
                for semdim, semdim_data in input_type_data.get("dimensions", {}).items():
                    ...
                    if score is not None:
                        # This will average scores if a model/input/word_type/semdim appears across multiple languages (e.g. in future use cases)
                        # For the current logic, it will just store the score.
                        if semdim not in model_scores[model_name][input_type][word_type_to_process]:
                            model_scores[model_name][input_type][word_type_to_process][semdim] = []
                        model_scores[model_name][input_type][word_type_to_process][semdim].append(score)

    # Finalize by averaging scores collected in lists
    for model_name, input_data in model_scores.items():
        for input_type, word_data in input_data.items():
            for word_type, semdim_data in word_data.items():
                for semdim, scores in semdim_data.items():
                    if scores:
                        model_scores[model_name][input_type][word_type][semdim] = {metric: np.mean(scores)}
                    else:
                        model_scores[model_name][input_type][word_type][semdim] = {metric: np.nan}
...
def aggregate_per_model(json_path, metric='macro_f1_score'):
    ...
    # Structure: {model: {word_type: {semdim: [scores]}}}
    model_scores_aggregated = {}

    # Aggregate scores for natural languages
    for lang in natural_languages:
        if lang not in data: continue
        for model_name, model_data in data[lang].items():
            if model_name not in model_scores_aggregated:
                model_scores_aggregated[model_name] = {'natural': {}, 'constructed': {}}
            
            if 'natural' in model_data:
                for input_type, input_type_data in model_data['natural'].items():
                    for semdim, semdim_data in input_type_data.get("dimensions", {}).items():
                        if semdim not in constructed_dims: continue
                        score = semdim_data.get(metric)
                        if score is not None:
                            if semdim not in model_scores_aggregated[model_name]['natural']:
                                model_scores_aggregated[model_name]['natural'][semdim] = []
                            model_scores_aggregated[model_name]['natural'][semdim].append(score)

    # Aggregate scores for constructed language
    for lang in constructed_languages:
        if lang not in data: continue
        for model_name, model_data in data[lang].items():
            if model_name not in model_scores_aggregated:
                model_scores_aggregated[model_name] = {'natural': {}, 'constructed': {}}

            if 'constructed' in model_data:
                for input_type, input_type_data in model_data['constructed'].items():
                    for semdim, semdim_data in input_type_data.get("dimensions", {}).items():
                        if semdim not in constructed_dims: continue
                        score = semdim_data.get(metric)
                        if score is not None:
                            if semdim not in model_scores_aggregated[model_name]['constructed']:
                                model_scores_aggregated[model_name]['constructed'][semdim] = []
                            model_scores_aggregated[model_name]['constructed'][semdim].append(score)
    
    # Calculate the mean for each model, word_type, and semdim
    final_model_scores = {}
    for model_name, word_type_data in model_scores_aggregated.items():
        final_model_scores[model_name] = {'natural': {}, 'constructed': {}}
        for word_type, semdim_data in word_type_data.items():
            for semdim, scores in semdim_data.items():
                if scores:
                    final_model_scores[model_name][word_type][semdim] = {metric: np.mean(scores)}
    ...
    for model_name, model_data in final_model_scores.items():
        if model_name.startswith('gpt-4o'):
            for semdim, score_data in model_data.get('natural', {}).items():
                if semdim not in gpt4o_natural: gpt4o_natural[semdim] = []
                gpt4o_natural[semdim].append(score_data[metric])
            for semdim, score_data in model_data.get('constructed', {}).items():
                if semdim not in gpt4o_constructed: gpt4o_constructed[semdim] = []
                gpt4o_constructed[semdim].append(score_data[metric])
        else:
            grouped_scores[model_name] = model_data

    if gpt4o_natural or gpt4o_constructed:
        grouped_scores['gpt-4o'] = {'natural': {}, 'constructed': {}}
        for semdim, scores in gpt4o_natural.items():
            if scores: grouped_scores['gpt-4o']['natural'][semdim] = {metric: np.mean(scores)}
        for semdim, scores in gpt4o_constructed.items():
            if scores: grouped_scores['gpt-4o']['constructed'][semdim] = {metric: np.mean(scores)}
...
def aggregate_macro_f1_by_inputtype(json_path, constructed_dims=constructed_dims, metric='macro_f1_score'):
    ...
    # Helper function to group models
    def group_models(models_data):
        grouped = {}
        for model_name, model_data in models_data.items():
            key = 'gpt-4o' if model_name.startswith('gpt-4o') else model_name
    ...
    for lang in natural_languages:
        ...
        grouped_models_data = group_models(data[lang])
        dim_scores_per_lang = {} # {semdim: [scores]}
        
        for model_group_name, model_data_list in grouped_models_data.items():
            for model_data in model_data_list:
                if 'natural' in model_data:
                    for input_type, input_type_data in model_data['natural'].items():
                        for semdim, semdim_data in input_type_data.get("dimensions", {}).items():
                            ...
                            score = semdim_data.get(metric)
                            if score is not None:
                                if semdim not in dim_scores_per_lang: dim_scores_per_lang[semdim] = []
                                dim_scores_per_lang[semdim].append(score)
        
        for semdim, scores in dim_scores_per_lang.items():
            if scores:
                lang_dim_scores[lang][semdim] = np.mean(scores)

    natural_final_scores = {}
    all_dims_natural = set(d for scores in lang_dim_scores.values() for d in scores.keys())
    for dim in all_dims_natural:
        lang_scores = [lang_dim_scores[lang].get(dim) for lang in natural_languages]
        valid_scores = [s for s in lang_scores if s is not None]
        if valid_scores:
    ...
    if 'art' in data:
        ...
        dim_scores_art = {} # {semdim: [scores]}

        for model_group_name, model_data_list in grouped_models_data_art.items():
            for model_data in model_data_list:
                if 'constructed' in model_data:
                    for input_type, input_type_data in model_data['constructed'].items():
                        for semdim, semdim_data in input_type_data.get("dimensions", {}).items():
                            ...
                            score = semdim_data.get(metric)
                            if score is not None:
                                if semdim not in dim_scores_art: dim_scores_art[semdim] = []
                                dim_scores_art[semdim].append(score)
        
        for semdim, scores in dim_scores_art.items():
            if scores:
                constructed_final_scores[semdim] = {metric: np.mean(scores)}

    # --- Final Assembly ---
    final_results = {
        "whole": {
            "constructed": constructed_final_scores,
            "natural": natural_final_scores
        }
    }
...
if __name__ == "__main__":
    ...
    metric = 'macro_f1_score'
    # The aggregation functions now implement the new logic.
    ...
    natural_model_scores = {model: data['natural'] for model, data in model_results.items() if data.get('natural')}
    constructed_model_scores = {model: data['constructed'] for model, data in model_results.items() if data.get('constructed')}
    ...
    plot_final_rq1_bar_comparison(
        ...
        input_type='whole_lang_avg' # Changed input_type for clearer file names
    )
    ...
    gpt4o_data = {} # {input_type: {word_type: {semdim: [scores]}}}
        ...
        if model_name.startswith('gpt-4o'):
            for input_type, word_data in model_data.items():
                if input_type not in gpt4o_data:
                    gpt4o_data[input_type] = {}
                for word_type, semdim_data in word_data.items():
                    if word_type not in gpt4o_data[input_type]:
                        gpt4o_data[input_type][word_type] = {}
                    for semdim, score_dict in semdim_data.items():
                        if semdim not in gpt4o_data[input_type][word_type]:
                            gpt4o_data[input_type][word_type][semdim] = []
                        gpt4o_data[input_type][word_type][semdim].append(score_dict[metric])
        else:
            processed_model_scores[model_name] = model_data
    
    if gpt4o_data:
        # Average the collected gpt-4o scores
        avg_gpt4o_data = {}
        for input_type, word_data in gpt4o_data.items():
            avg_gpt4o_data[input_type] = {}
            for word_type, semdim_data in word_data.items():
                avg_gpt4o_data[input_type][word_type] = {}
                for semdim, scores in semdim_data.items():
                    if scores:
                        avg_gpt4o_data[input_type][word_type][semdim] = {metric: np.mean(scores)}
        processed_model_scores['gpt-4o'] = avg_gpt4o_data
    # --- End grouping ---
