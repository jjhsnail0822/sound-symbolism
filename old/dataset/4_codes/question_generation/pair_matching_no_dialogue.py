...
def generate_datasets(language, task, experiment_name):
    ...
    for subject_word in dialogues:
        for dialogue in subject_word['dialogues'][:1]: # only use the first dialogue
            ...
            # create the prompt
            question = prompts[TASK][EXPERIMENT_NAME][LANGUAGE][PROMPT_ROLE].format(word=word_text, meaning=meaning_text, options=option_string, MAX_OPTION=MAX_OPTION, MASKING_WORD=MASKING_WORD)
    ...
    if not os.path.exists('dataset/3_questions/nat/understanding_pair_matching_no_dialogue'):
        os.makedirs('dataset/3_questions/nat/understanding_pair_matching_no_dialogue')
    with open(f'dataset/3_questions/nat/understanding_pair_matching_no_dialogue/{EXPERIMENT_NAME}-{LANGUAGE}.json', 'w', encoding='utf-8') as f:
