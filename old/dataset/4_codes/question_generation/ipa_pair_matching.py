...
def generate_datasets(language, task, experiment_name):
    ...
    IS_OPTION_MEANING = True if EXPERIMENT_NAME == "ipa_unmasked_word_to_meaning_mcq" else False # whether the options are meaning or word
    MASKING = False if EXPERIMENT_NAME == "ipa_unmasked_word_to_meaning_mcq" else True # whether to mask the subject word
    MASKING_WORD = "[__]"
    MAX_OPTION = 4
    ...
    for subject_word in dialogues:
        for dialogue in subject_word['dialogues']:
            ...
            # create the prompt
            question = prompts[TASK][EXPERIMENT_NAME][LANGUAGE][PROMPT_ROLE].format(word=subject_word['ipa'].replace(" ",""), meaning=meaning_text, dialogue=dialogue_text, options=option_string, MAX_OPTION=MAX_OPTION, MASKING_WORD=MASKING_WORD)
    ...
    with open(f'dataset/3_questions/nat/understanding_ipa_pair_matching/{EXPERIMENT_NAME}-{LANGUAGE}.json', 'w', encoding='utf-8') as f:
...
if __name__ == "__main__":
    for lang in ["en", "fr", "ja", "ko"]:
        generate_datasets(language=lang, task="understanding", experiment_name="ipa_unmasked_word_to_meaning_mcq")
        generate_datasets(language=lang, task="understanding", experiment_name="ipa_masked_meaning_to_word_mcq")
